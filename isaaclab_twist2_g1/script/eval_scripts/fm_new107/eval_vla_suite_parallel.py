...
from fm107_contract import validate_checkpoint
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Single-GPU Flow Matching motion-token raw107 evaluation scheduler')
    ...
    if args.num_workers != 1:
        raise ValueError('fm_new107 single-GPU mode requires --num_workers=1')
    ...
    if len(server_devices) != 1:
        raise ValueError(
            'fm_new107 single-GPU mode requires exactly one GPU; '
    ...
    if not args.disable_gpu_lock:
        ...
        gpu_lock_fp = _acquire_exclusive_lock(
            Path(f"/tmp/humanoidarena_fm_new107_gpu_{gpu_label}.lock"),
