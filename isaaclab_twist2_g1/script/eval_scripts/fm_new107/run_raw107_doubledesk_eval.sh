...
# Fair DoubleDesk evaluation for SONIC Flow Matching raw107 checkpoints. This
# wrapper only selects the matching robot/policy interface; it adds no task
# controller.
...
RESULTS_ROOT="${RESULTS_ROOT:-${SCRIPT_DIR}/eval_results/fm_new107_doubledesk_$(date +%Y%m%d_%H%M%S)}"

for checkpoint in "${CHECKPOINTS[@]}"; do
  if [[ ! -d "${checkpoint}" ]]; then
    echo "[fm_new107] checkpoint not found: ${checkpoint}" >&2
    exit 2
  fi
  ...
  RESULTS_DIR="${RESULTS_ROOT}/${model_label}" \
    bash "${SCRIPT_DIR}/run_vla_eval.sh" "${checkpoint}"
done
