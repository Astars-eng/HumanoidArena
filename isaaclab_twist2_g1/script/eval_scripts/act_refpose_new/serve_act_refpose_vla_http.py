...
class LeRobotServerState:
    def __init__(self, policy_dir: Path, device_name: str):
        self.policy_dir = policy_dir.expanduser().resolve()
        self.instance_token = os.environ.get("ACT_REFPOSE_NEW_SERVER_INSTANCE_TOKEN", "").strip()
        (
            self.config,
            self.policy,
            self.preprocessor,
    ...
    def _validate_act_refpose_contract(self) -> None:
        ...
        errors = []
        if policy_type != "act":
            errors.append(f"type={policy_type!r}, expected 'act'")
        if state_keys != _EXPECTED_STATE_KEYS:
            errors.append(f"state_keys={state_keys!r}, expected {_EXPECTED_STATE_KEYS!r}")
        if action_keys != _EXPECTED_ACTION_KEYS:
            errors.append(f"action_keys={action_keys!r}, expected {_EXPECTED_ACTION_KEYS!r}")
        if obs_delta_sequence != _EXPECTED_OBS_DELTA_SEQUENCE:
            errors.append(
                f"obs_delta_sequence={obs_delta_sequence!r}, expected {_EXPECTED_OBS_DELTA_SEQUENCE!r}"
            )
        if self.expected_history_steps != len(_EXPECTED_OBS_DELTA_SEQUENCE):
            errors.append(
                f"n_obs_steps={self.expected_history_steps}, expected {len(_EXPECTED_OBS_DELTA_SEQUENCE)}"
            )
        if self.expected_state_shape != _EXPECTED_STATE_SHAPE:
            errors.append(
                f"observation.state shape={self.expected_state_shape}, expected {_EXPECTED_STATE_SHAPE}"
            )
        if self.expected_action_shape != _EXPECTED_ACTION_SHAPE:
            errors.append(f"action shape={self.expected_action_shape}, expected {_EXPECTED_ACTION_SHAPE}")
        if self.expected_image_shape != _EXPECTED_IMAGE_SHAPE:
            errors.append(
                f"observation.images.front shape={self.expected_image_shape}, "
                f"expected {_EXPECTED_IMAGE_SHAPE}"
            )
        if chunk_size != _EXPECTED_CHUNK_STEPS:
            errors.append(f"chunk_size={chunk_size}, expected {_EXPECTED_CHUNK_STEPS}")
        if n_action_steps != _EXPECTED_CHUNK_STEPS:
            errors.append(f"n_action_steps={n_action_steps}, expected {_EXPECTED_CHUNK_STEPS}")

        component_dims = [component_slice.stop - component_slice.start for _, component_slice in self.state_component_slices]
        if component_dims != _EXPECTED_STATE_COMPONENT_DIMS:
            errors.append(
                f"state component dims={component_dims}, expected {_EXPECTED_STATE_COMPONENT_DIMS}"
            )
        if errors:
            raise ValueError(
                "Checkpoint is incompatible with act_refpose_new:\n  - " + "\n  - ".join(errors)
            )

        print(
            "[act_refpose_new_server] contract validated "
            "input=10x64(joint_pos29+joint_vel29+ang_vel_b3+gravity3) "
            "output=25x40(refpose_v3_1)",
            flush=True,
        )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            try:
                ...
                image = resize_rgb_to_policy_shape(image, state.expected_image_shape)
