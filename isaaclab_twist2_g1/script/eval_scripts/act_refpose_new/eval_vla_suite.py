...
def _wait_for_server_ready(
    ...
    *,
    ...
    expected_instance_token = str(
        getattr(server_process, "act_refpose_new_instance_token", "") or ""
    )
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if not expected_instance_token or health.get("instance_token") != expected_instance_token:
                raise RuntimeError(
                    f"Server instance mismatch at {base_url}: the port is not owned by the process "
            ...
            if mismatches:
                raise RuntimeError(f"ACT checkpoint contract mismatch at {base_url}: {mismatches}")
            client.reset()
            print(
                f"[act_refpose_new] server verified url={base_url} model={actual_path} "
                "input=10x64 image=3x224x224 output=25x40"
            )
            return
...
def _start_server(args, model_path: str, log_path: Path):
    ...
    server_env["ACT_REFPOSE_NEW_SERVER_INSTANCE_TOKEN"] = instance_token
    cmd = [
        args.server_python,
        "-u",
        str(server_script),
    ...
    process.act_refpose_new_instance_token = instance_token
    ...
    *,
    ...
    if result_json.exists():
        ...
        result = {
            ...
            "act_refpose_history_steps": int(args.act_refpose_history_steps),
            "act_refpose_execute_steps": int(args.act_refpose_execute_steps),
            "act_refpose_record_full_chunks": bool(args.act_refpose_record_full_chunks),
        }
...
def _run_episode(
    ...
    cmd = [
        ...
        str(args.act_refpose_history_steps),
        "--act_refpose_execute_steps",
        str(args.act_refpose_execute_steps),
        "--act_refpose_record_full_chunks",
        str(args.act_refpose_record_full_chunks),
        "--device",
        args.isaac_device,
        "--enable_cameras",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Non-parallel aligned ACT ref-pose DoubleDesk evaluation suite")
    parser.add_argument("--task", type=str, default=TASK_DOUBLE_DESK)
    parser.add_argument("--env_config_yaml", type=str, default="tasks/common_env_config/doubledesk_sonic.yaml")
    ...
    parser.add_argument("--act_refpose_history_steps", type=int, default=10)
    parser.add_argument("--act_refpose_execute_steps", type=int, default=5)
    parser.add_argument("--act_refpose_record_full_chunks", type=int, choices=[0, 1], default=1)
