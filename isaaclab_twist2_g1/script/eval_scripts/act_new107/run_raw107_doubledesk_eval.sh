...
# Fair DoubleDesk evaluation for SONIC raw107 checkpoints.  This wrapper only
...
DEFAULT_CHECKPOINTS=(
  "/DATA/disk0/fym/vla/outputs/act_humanoidarena_sonic_20260812_145530/HOI_double_desk/checkpoints/160000/pretrained_model"
)
...
RESULTS_ROOT="${RESULTS_ROOT:-${SCRIPT_DIR}/eval_results/raw107_doubledesk_$(date +%Y%m%d_%H%M%S)}"

for checkpoint in "${CHECKPOINTS[@]}"; do
  if [[ ! -d "${checkpoint}" ]]; then
    echo "[act_new107] checkpoint not found: ${checkpoint}" >&2
    exit 2
  fi
