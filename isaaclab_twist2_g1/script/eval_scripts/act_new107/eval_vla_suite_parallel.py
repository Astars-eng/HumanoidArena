...
def main() -> int:
    parser = argparse.ArgumentParser(description='Single-GPU ACT motion-token raw107 evaluation scheduler')
    ...
    if args.num_workers != 1:
        raise ValueError('act_new107 single-GPU mode requires --num_workers=1')
    ...
    if len(server_devices) != 1:
        raise ValueError(
            'act_new107 single-GPU mode requires exactly one GPU; '
    ...
    if not args.disable_gpu_lock:
        ...
        gpu_lock_fp = _acquire_exclusive_lock(
            Path(f"/tmp/humanoidarena_act_new107_gpu_{gpu_label}.lock"),
