#!/usr/bin/env python3
"""Fail-fast validation for ACT SONIC motion-token raw107 checkpoints."""
...
def main() -> int:
    ...
    print(
        "[act_new107] checkpoint contract OK: "
        f"type={config['type']} state=29+29+3+3=64 history={config['n_obs_steps']} "
        f"image=224x224 action=29+motion_token64+7+7=107 chunk={config['chunk_size']}"
    )
    return 0
