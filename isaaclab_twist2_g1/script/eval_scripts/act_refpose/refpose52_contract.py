#!/usr/bin/env python3
"""Fail-fast contract validation for ACT RefPose v3.1 hand14 checkpoints."""
...
def validate_checkpoint(path: str | Path) -> dict:
    ...
    if not config_path.is_file():
        raise FileNotFoundError(f"ACT RefPose checkpoint config not found: {config_path}")
    config = json.loads(config_path.read_text(encoding="utf-8"))

    _require_equal("policy type", config.get("type"), "act")
    ...
    _require_equal("action_keys", config.get("action_keys"), ["action"])
...
def main() -> int:
    ...
    print(
        "[act_refpose] checkpoint contract OK: "
        f"type={config['type']} input=10x64 image=224x224 "
        f"output=25x52 layout=root9+joint29+left_hand7+right_hand7 "
        f"execute_prefix_configurable"
    )
    return 0
