#!/usr/bin/env python3
"""Fail-fast validation for Diffusion SONIC motion-token raw107 checkpoints."""
...
def validate_checkpoint(path: str | Path) -> dict:
    ...
    if not config_path.is_file():
        raise FileNotFoundError(f"Diffusion raw107 checkpoint config not found: {config_path}")
    config = json.loads(config_path.read_text(encoding="utf-8"))

    _require_equal("policy type", config.get("type"), "diffusion")
    _require_equal("n_obs_steps", config.get("n_obs_steps"), 2)
    _require_equal("horizon", config.get("horizon"), 32)
    _require_equal("n_action_steps", config.get("n_action_steps"), 25)
    _require_equal("drop_n_last_frames", config.get("drop_n_last_frames"), 6)
