...
# Fair DoubleDesk evaluation for SONIC Diffusion raw107 checkpoints. This
...
RESULTS_ROOT="${RESULTS_ROOT:-${SCRIPT_DIR}/eval_results/dp_new107_doubledesk_$(date +%Y%m%d_%H%M%S)}"

for checkpoint in "${CHECKPOINTS[@]}"; do
  if [[ ! -d "${checkpoint}" ]]; then
    echo "[dp_new107] checkpoint not found: ${checkpoint}" >&2
    exit 2
  fi
