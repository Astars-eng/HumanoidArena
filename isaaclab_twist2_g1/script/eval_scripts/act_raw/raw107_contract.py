#!/usr/bin/env python3
"""Fail-fast validation for ACT/STREAM SONIC raw107 checkpoints."""
...
def validate_checkpoint(path: str | Path) -> dict:
    ...
    if policy_type not in {"act", "stream"}:
        raise ValueError(f"policy type: expected 'act' or 'stream', got {policy_type!r}")
...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit(f"usage: {Path(sys.argv[0]).name} /path/to/pretrained_model")
    config = validate_checkpoint(sys.argv[1])
    print(
        "[act_raw] checkpoint contract OK: "
        f"type={config['type']} state=29+29+3+3=64 history={config['n_obs_steps']} "
        f"image=224x224 action=29+64+7+7=107 chunk={config['chunk_size']}"
    )
    return 0
