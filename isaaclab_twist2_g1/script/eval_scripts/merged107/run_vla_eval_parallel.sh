...
# The merged dataset/checkpoints use raw64 proprioception followed by the
# previous actually executed raw29 action, all in DFS/MuJoCo joint order.
SONIC_VLA_STATE_FORMAT="raw93"
SONIC_RAW107_BODY_SOURCE="${SONIC_RAW107_BODY_SOURCE:-native_decoder}"
SONIC_RAW_STATE_JOINT_ORDER="mujoco"
...
SERVER_VERBATIM_TASK="1"
SERVER_STRETCH_IMAGE_TO_POLICY_SHAPE="1"
...
# Allow multiple merged107 evaluations to share one physical GPU by default.
# Set GPU_LOCK=1 when an exclusive evaluation process is desired.
GPU_LOCK="${GPU_LOCK:-0}"
TLS_CERT_FILE="${TLS_CERT_FILE:-}"
TLS_KEY_FILE="${TLS_KEY_FILE:-}"

DEFAULT_CHECKPOINTS_CSV="/DATA/disk0/fym/vla/outputs/act_humanoidarena_sonic_merged_107_0820_fym/checkpoints/100000/pretrained_model,/DATA/disk0/fym/vla/outputs/dp_humanoidarena_sonic_merged_act107_0820_fym/checkpoints/100000/pretrained_model,/DATA/disk0/fym/vla/outputs/fm_humanoidarena_sonic_merged_act107_0820_fym/checkpoints/100000/pretrained_model"
if [[ -n "${MODEL_PATH:-}" && -z "${MODEL_PATHS_CSV:-}" ]]; then
  ...
  MODEL_PATHS_CSV="${DEFAULT_CHECKPOINTS_CSV}"
fi
MODEL_ROOT="${MODEL_ROOT:-$DEFAULT_BATCH_MODEL_ROOT}"
MODEL_GLOB="${MODEL_GLOB:-}"
RESULTS_TAG="${RESULTS_TAG:-merged107}"
...
POLICY_TASK="${POLICY_TASK:-$("${EVAL_PYTHON}" "${RUN_SCRIPT_DIR}/task_naming.py" prompt "${TASK_NAME}")}"
...
echo "Task: ${TASK_NAME}"
echo "Policy prompt: ${POLICY_TASK}"
...
echo "Observation state format: ${SONIC_VLA_STATE_FORMAT}"
...
ARGS=(
  ...
  --sonic_vla_state_format "${SONIC_VLA_STATE_FORMAT}"
