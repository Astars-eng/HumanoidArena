...
from task_naming import model_label_for_task_checkpoint, policy_prompt_for_task
...
def _server_log_tail(log_path: Path | None, max_lines: int = 40) -> str:
    if log_path is None:
        return ""
    try:
...
def _wait_for_server_ready(
    ...
    *,
    ...
    while time.time() < deadline:
        if process is not None:
            ...
            if return_code is not None:
                log_tail = _server_log_tail(log_path)
                detail = f"\nServer log ({log_path}):\n{log_tail}" if log_tail else ""
                raise RuntimeError(
                    f"LeRobot server exited before becoming ready with code {return_code}."
                    f"{detail}"
                )
        try:
            client.reset()
    ...
    detail = f"\nServer log ({log_path}):\n{log_tail}" if log_tail else ""
    raise RuntimeError(
        f"LeRobot server not ready at {base_url} after {timeout_s:.1f}s: {last_error}"
        f"{detail}"
    )
...
def _start_server(args, model_path: str, log_path: Path):
    ...
    for remap in args.server_checkpoint_ref_remap:
        cmd.extend(["--checkpoint-ref-remap", remap])
    if args.server_verbatim_task:
        cmd.append("--verbatim-task")
    if args.server_stretch_image_to_policy_shape:
    ...
    *,
    ...
    if result_json.exists():
        ...
        result = {
            ...
            "sonic_vla_action_format": args.sonic_vla_action_format,
            "sonic_vla_state_format": args.sonic_vla_state_format,
            "sonic_raw107_body_source": args.sonic_raw107_body_source,
            "sonic_raw_state_joint_order": args.sonic_raw_state_joint_order,
...
def _run_episode(
    ...
    cmd = [
        ...
        "--sonic_vla_state_format",
        args.sonic_vla_state_format,
...
def main() -> int:
    ...
    parser.add_argument(
        "--sonic_vla_state_format",
        type=str,
        default=os.environ.get("SONIC_VLA_STATE_FORMAT", "raw93"),
        choices=["raw93"],
        help="Merged raw93 observation contract.",
    )
    parser.add_argument(
        "--sonic_raw_state_joint_order",
        type=str,
        default=os.environ.get("SONIC_RAW_STATE_JOINT_ORDER", "mujoco"),
        choices=["mujoco"],
        help="29-D joint order expected by the checkpoint observation.",
    )
    ...
    parser.add_argument(
        ...
        help="LeRobot src directory matching the checkpoint implementation.",
    )
    parser.add_argument(
        "--server_checkpoint_ref_remap",
        action="append",
        default=[],
        help="Checkpoint-internal OLD=NEW path remap passed to the policy server.",
    )
    parser.add_argument(
        "--server_verbatim_task",
        action="store_true",
        default=False,
        help="Pass the evaluation task string to the policy without alias translation.",
    )
    parser.add_argument(
        "--server_stretch_image_to_policy_shape",
        action="store_true",
        default=False,
        help="Match converter-time direct image resizing instead of model aspect padding.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(os.environ.get("SERVER_READY_TIMEOUT", "1800")),
        help="Seconds to allow for loading large VLA checkpoints before the HTTP readiness check fails.",
    )
    args = parser.parse_args()
    expected_policy_task = policy_prompt_for_task(args.task)
    if args.policy_task != expected_policy_task:
        raise ValueError(
            f"policy_task must match the merged training prompt {expected_policy_task!r}; "
            f"got {args.policy_task!r}"
        )
    if not args.server_verbatim_task:
        raise ValueError("merged107 requires --server_verbatim_task")
    if not args.server_stretch_image_to_policy_shape:
        raise ValueError("merged107 requires --server_stretch_image_to_policy_shape")
    ...
    try:
        for model_path in args.model_paths:
            ...
            model_label = _sanitize_label(
                model_label_for_task_checkpoint(args.task, resolved_model_path)
            )
            ...
            print(
                f"[eval_vla_suite] waiting up to {args.server_ready_timeout:.1f}s "
                f"for {server_url}; pid={server_proc.pid} log={server_log_path}"
            )
            try:
