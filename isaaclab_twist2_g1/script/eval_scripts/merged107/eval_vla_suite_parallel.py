...
def main() -> int:
    ...
    parser.add_argument('--sonic_vla_state_format', type=str, default='raw93', choices=['raw93'])
    parser.add_argument(
    ...
    if args.num_workers != 1:
        raise ValueError('merged107 single-GPU mode requires --num_workers=1')
    ...
    if len(server_devices) != 1:
        raise ValueError(
            'merged107 single-GPU mode requires exactly one GPU; '
    ...
    if not args.disable_gpu_lock:
        ...
        gpu_lock_fp = _acquire_exclusive_lock(
            Path(f"/tmp/humanoidarena_merged107_gpu_{gpu_label}.lock"),
            f"GPU {server_devices[0]}",
        )
    ...
    _held_locks = (run_lock_fp, gpu_lock_fp)
