...
from task_naming import policy_prompt_for_task
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Single-episode or seed-batched merged107 evaluation")
    parser.add_argument("--task", type=str, default=TASK_DOUBLE_DESK)
    parser.add_argument(
        "--policy_task",
        type=str,
        required=True,
        help="Exact natural-language prompt used in the merged training dataset.",
    )
    ...
    parser.add_argument(
        ...
        choices=["raw93"],
        help="Merged policy state: raw64 proprioception plus previous executed raw29 action.",
    )
    parser.add_argument(
        ...
        choices=["mujoco"],
        help="29-D joint order supplied to the raw93 checkpoint observation.",
    )
...
def _normalize_control_routing(args):
    ...
    args.language_instruction = args.policy_task
...
def _build_result_payload(args_cli, spec: dict, model_label: str, server_url: str, *, success: bool, failure_reason: str, step_idx: int, terminal_step_idx: int, final_reward: float, final_reward_scaled: float, max_reward: float, max_reward_scaled: float, video_path: str, started_at: float, error: str = "", terminal_detail: str = "", env=None) -> dict:
    ...
    payload = {
        ...
        "sonic_vla_state_format": str(args_cli.sonic_vla_state_format),
        ...
        "policy_task": str(args_cli.policy_task),
...
def main() -> int:
    ...
    expected_policy_task = policy_prompt_for_task(args_cli.task)
    if args_cli.policy_task != expected_policy_task:
        raise ValueError(
            f"policy_task must match the merged training prompt {expected_policy_task!r}; "
            f"got {args_cli.policy_task!r}"
        )
