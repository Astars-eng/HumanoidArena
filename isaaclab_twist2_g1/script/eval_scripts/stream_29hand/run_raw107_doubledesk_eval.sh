...
# Fair DoubleDesk evaluation for raw29 + independent Dex3 checkpoints. This wrapper only
...
if [[ "$#" -eq 0 ]]; then
  echo "usage: $(basename "$0") /path/to/hand-checkpoint[/pretrained_model] [...]" >&2
  exit 2
fi
CHECKPOINTS=("$@")
...
RESULTS_ROOT="${RESULTS_ROOT:-${SCRIPT_DIR}/eval_results/raw29hand_doubledesk_$(date +%Y%m%d_%H%M%S)}"

for checkpoint in "${CHECKPOINTS[@]}"; do
  if [[ ! -d "${checkpoint}" ]]; then
    echo "[stream_29hand] checkpoint not found: ${checkpoint}" >&2
    exit 2
  fi
