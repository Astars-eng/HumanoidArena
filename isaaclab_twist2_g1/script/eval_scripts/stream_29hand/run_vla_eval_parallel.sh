...
SERVER_SCRIPT="${SERVER_SCRIPT:-${RUN_SCRIPT_DIR}/serve_lerobot_vla_http.py}"
...
if [[ -n "${MODEL_PATH:-}" && -z "${MODEL_PATHS_CSV:-}" ]]; then
  ...
  echo "Error: stream_29hand requires an explicit hand-enabled checkpoint." >&2
  echo "Set MODEL_PATH, MODEL_PATHS_CSV, MODEL_PATHS_FILE, or MODEL_ROOT." >&2
  exit 2
fi
...
DEFAULT_REPEATS_PER_SEED="$(load_test_default_from_yaml "${ENV_CONFIG_YAML}" repeats_per_seed 20)"
REPEATS_PER_SEED="${REPEATS_PER_SEED:-${DEFAULT_REPEATS_PER_SEED}}"
DEFAULT_SEEDS="$(load_test_default_from_yaml "${ENV_CONFIG_YAML}" seeds "0 1 2")"
...
DEFAULT_RESULTS_TAG="${TASK_RESULT_NAME}_stream_29hand"
...
echo "Independent hand actions: required (action.left_hand[7] + action.right_hand[7])"
...
LEROBOT_REQUIRE_HAND_ACTIONS="1" \
SONIC_VLA_ACTION_FORMAT="raw29" \
