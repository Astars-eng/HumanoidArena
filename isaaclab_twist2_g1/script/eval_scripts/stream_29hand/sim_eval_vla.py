...
from task_naming import episode_video_stem
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Single-episode or seed-batched Stream raw29 evaluation")
    ...
    parser.add_argument(
        ...
        help="29-D joint order supplied to the raw64 checkpoint observation.",
    )
...
def _run_episode_once(simulation_app, env, env_cfg, action_provider, controller, args_cli, spec: dict, SimpleVideoRecorder):
    ...
    episode_name = episode_video_stem(
        args_cli.task,
        model_label,
        spec["seed"],
        spec["repeat_idx"],
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            exit_code = 130
            print(f"[sim_eval_vla] startup interrupted: {_INTERRUPT_REASON}")
