...
class LeRobotServerState:
    def __init__(
        ...
        *,
        ...
        self.hand_action_keys = list(getattr(self.config, "hand_action_keys", ()) or ())
        self.hand_action_key_dims = {
            str(key): int(value)
            for key, value in (getattr(self.config, "hand_action_key_dims", {}) or {}).items()
        }
        missing_hand_dims = [key for key in self.hand_action_keys if key not in self.hand_action_key_dims]
        if missing_hand_dims:
            raise ValueError(
                "Hand-action checkpoint is missing dimensions for "
                f"{missing_hand_dims}: {self.hand_action_key_dims}"
            )
        if self.hand_action_keys and not callable(
            getattr(self.policy, "get_last_selected_hand_action", None)
        ):
            raise ValueError(
                "Hand-action checkpoint requires policy.get_last_selected_hand_action()"
            )
        if self.hand_action_keys and not callable(
            getattr(self.policy, "get_last_predicted_hand_action_chunk", None)
        ):
            raise ValueError(
                "Hand-action checkpoint requires policy.get_last_predicted_hand_action_chunk()"
            )
        ...
        print(
            "[lerobot_vla_server] independent hand actions "
            f"keys={self.hand_action_keys} dims={self.hand_action_key_dims}",
            flush=True,
        )
    ...
    def _split_hand_action(self, hand_action, *, source: str) -> dict[str, np.ndarray]:
        if not self.hand_action_keys:
            return {}
        if hand_action is None:
            raise RuntimeError(f"{source} returned no hand action for {self.hand_action_keys}")
        if not isinstance(hand_action, torch.Tensor):
            hand_action = torch.as_tensor(hand_action)
        hand_action = hand_action.detach().cpu().to(torch.float32)
        if hand_action.ndim < 2 or hand_action.shape[0] != 1:
            raise ValueError(
                f"{source} must return a single-batch hand tensor, got {tuple(hand_action.shape)}"
            )
        expected_dim = sum(self.hand_action_key_dims[key] for key in self.hand_action_keys)
        if hand_action.shape[-1] != expected_dim:
            raise ValueError(
                f"{source} hand width must be {expected_dim}, got {tuple(hand_action.shape)}"
            )
        if not torch.isfinite(hand_action).all():
            raise ValueError(f"{source} hand action contains NaN or Inf")

        unbatched = hand_action.squeeze(0).numpy()
        split: dict[str, np.ndarray] = {}
        offset = 0
        for key in self.hand_action_keys:
            end = offset + self.hand_action_key_dims[key]
            split[key] = np.ascontiguousarray(unbatched[..., offset:end])
            offset = end
        return split
    ...
    def infer(self, observation: dict, robot_type: str, task: str | None):
        with self.lock:
            with torch.inference_mode(), self._amp_context():
                ...
                hand_actions = self._split_hand_action(
                    self.policy.get_last_selected_hand_action() if self.hand_action_keys else None,
                    source="policy.get_last_selected_hand_action()",
                )
                return self.postprocessor(action), hand_actions
    ...
    def infer_chunk(
        ...
        with self.lock:
            with torch.inference_mode(), self._amp_context():
                ...
                hand_action_chunk = self._split_hand_action(
                    self.policy.get_last_predicted_hand_action_chunk()
                    if self.hand_action_keys
                    else None,
                    source="policy.get_last_predicted_hand_action_chunk()",
                )
                return raw_action_chunk.detach().cpu().to(torch.float32).numpy(), hand_action_chunk
        ...
        def do_POST(self):
            try:
                ...
                if bool(payload.get("return_chunk", False)):
                    action_chunk, hand_action_chunk = state.infer_chunk(
                        observation, robot_type, task_instruction
                    )
                    ...
                    if hand_action_chunk:
                        response["hand_actions"] = {
                            key: values[0].tolist() for key, values in hand_action_chunk.items()
                        }
                        response["hand_action_chunk"] = {
                            key: values.tolist() for key, values in hand_action_chunk.items()
                        }
                    log_suffix = f"chunk_size={action_chunk.shape[0]} first_action={first_action.tolist()}"
                else:
                    action, hand_actions = state.infer(observation, robot_type, task_instruction)
                    ...
                    if hand_actions:
                        response["hand_actions"] = {
                            key: values.tolist() for key, values in hand_actions.items()
                        }
                    log_suffix = (
                        f"action={action.tolist()} "
                        f"hand_keys={list(hand_actions)}"
                    )
