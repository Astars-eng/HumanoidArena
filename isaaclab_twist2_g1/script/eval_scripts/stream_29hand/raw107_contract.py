#!/usr/bin/env python3
"""Fail-fast validation for Stream raw29 + independent Dex3 checkpoints."""
...
EXPECTED_HAND_ACTION_KEYS = ["action.left_hand", "action.right_hand"]
...
def validate_checkpoint(path: str | Path) -> dict:
    ...
    _require_equal(
        "hand_action_keys",
        config.get("hand_action_keys"),
        EXPECTED_HAND_ACTION_KEYS,
    )
    ...
    _require_equal(
        "hand_action_key_dims",
        config.get("hand_action_key_dims"),
        EXPECTED_HAND_ACTION_KEY_DIMS,
    )
    return config
...
def main() -> int:
    ...
    print(
        "[stream_29hand] checkpoint contract OK: "
        f"type={config['type']} state=29+29+3+3+29=93 history={config['n_obs_steps']} "
        "image=224x224 body=29(DFS/MuJoCo raw) hands=7+7(Dex3 targets) "
