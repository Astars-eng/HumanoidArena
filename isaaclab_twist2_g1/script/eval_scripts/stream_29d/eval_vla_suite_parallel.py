...
from raw107_contract import validate_checkpoint
from task_naming import model_label_for_task_checkpoint
...
def _acquire_exclusive_lock(path: Path, description: str):
    lock_fp = open(path, "w", encoding="utf-8")
    try:
        fcntl.flock(lock_fp, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError as exc:
        lock_fp.close()
        raise RuntimeError(f"{description} is already in use (lock={path})") from exc
    return lock_fp
...
def _resolve_worker_isaac_device(base_isaac_device: str, server_device: str) -> str:
    ...
    if lowered == "cpu":
        return token
    # A bare ``cuda`` means "follow the server GPU" for backwards
    # compatibility.  An explicit physical ordinal must be preserved so large
    # Stream checkpoints can keep Isaac/PhysX on a different GPU.
    if lowered == "cuda":
        return server_device
    if token.isdigit():
        return f"cuda:{token}"
...
def _build_model_label(model_path: str, task_name: str) -> str:
    return _sanitize_label(model_label_for_task_checkpoint(task_name, model_path))
...
def _build_jobs(args) -> tuple[list[dict], dict[str, str]]:
    ...
    for model_path in args.model_paths:
        ...
        model_label = _build_model_label(resolved_model_path, args.task)
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Stream DFS/MuJoCo raw29 evaluation scheduler')
    ...
    parser.add_argument('--server_port_mode', choices=['auto', 'fixed'], default='auto')
    ...
    if args.num_workers != 1:
        raise ValueError('stream_new107 single-GPU mode requires --num_workers=1')
    if not 1 <= args.server_port_base <= 65535:
        raise ValueError('--server_port_base must be in [1, 65535]')
    if args.server_port_max and not args.server_port_base <= args.server_port_max <= 65535:
    ...
    if len(server_devices) != 1:
        raise ValueError(
            'stream_new107 requires exactly one server GPU; '
            'set --server_gpu_ids to one physical index such as 0'
        )
    isaac_device = _resolve_worker_isaac_device(args.isaac_device, server_devices[0])
    args.isaac_device = isaac_device
    if isaac_device == server_devices[0] and not args.allow_shared_gpu:
        raise ValueError(
            'Stream float32 and Isaac Sim cannot safely share one 24 GiB GPU. '
            f'Both resolved to {isaac_device}. Set ISAAC_GPU_ID to a different physical GPU '
            '(or pass --isaac_device cuda:N), or explicitly opt in with --allow_shared_gpu.'
        )
    gpu_lock_fp = None
    isaac_gpu_lock_fp = None
    if not args.disable_gpu_lock:
        gpu_label = _sanitize_label(server_devices[0])
        gpu_lock_fp = _acquire_exclusive_lock(
            Path(f"/tmp/humanoidarena_stream_new107_gpu_{gpu_label}.lock"),
            f"Stream server GPU {server_devices[0]}",
        )
        if isaac_device.startswith('cuda:') and isaac_device != server_devices[0]:
            isaac_gpu_label = _sanitize_label(isaac_device)
            isaac_gpu_lock_fp = _acquire_exclusive_lock(
                Path(f"/tmp/humanoidarena_stream_new107_gpu_{isaac_gpu_label}.lock"),
                f"Isaac GPU {isaac_device}",
            )
    ...
    # Keep the lock file descriptors alive until the scheduler exits.
    _held_locks = (run_lock_fp, gpu_lock_fp, isaac_gpu_lock_fp)
